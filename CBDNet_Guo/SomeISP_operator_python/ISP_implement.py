...
class ISP:
    ...
    def cbdnet_noise_generate_srgb(self, img):
        # To note that opencv store image in BGR,
        # When apply to color tranfer, BGR should be transfer to RGB
        img_rgb = img
        # -------- INVERSE ISP PROCESS -------------------
        print("starting inverse ISP process")
        ...
        print("adding Poisson-Gaussian noise")
        img_mosaic_noise = self.add_PG_noise(img_mosaic)
        # img_mosaic_noise = self.add_PG_noise(img_mosaic, sigma_s=0.01, sigma_c=0.02)

        # -------- ISP PROCESS --------------------------
        print("starting ISP process")
